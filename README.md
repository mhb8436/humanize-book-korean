...
`install.sh`(macOS/Linux) 또는 `install.ps1`(Windows)을 쓰면 스킬·에이전트·명령·도메인 자산이 한 번에 설치됩니다. **글로벌(`~/.claude`)로 설치하면 터미널·Claude Desktop의 Claude Code 양쪽에서, 모든 책 프로젝트에서 자동 인식**됩니다.

```bash
git clone https://github.com/{user}/humanize-book-korean.git ~/Workspaces/humanize-book-korean
cd ~/Workspaces/humanize-book-korean

# macOS / Linux
scripts/install.sh --global                 # 글로벌 설치 (모든 책에서 사용)
scripts/install.sh ~/Workspaces/your-book   # 특정 책 프로젝트에만 설치
#   기본은 심볼릭 링크(--link). 독립 사본은 --copy.
```

```powershell
# Windows (네이티브, WSL 불필요)
.\scripts\install.ps1 -Global               # 글로벌 설치 (기본: 사본 복사)
.\scripts\install.ps1 C:\path\to\your-book  # 특정 책 프로젝트에만 설치
#   기본은 -Copy. 심볼릭 링크는 -Link (개발자 모드/관리자 권한 필요).
```

도메인 자산(`examples/`)은 스킬 폴더 안에 포함되어 함께 설치됩니다. OS 무관 원클릭 배포가 필요하면 향후 Claude Code 플러그인(`/plugin install`)으로 확장할 수 있습니다.
...
본 스킬은 책마다 다른 **도메인 용어집 · 페르소나**를 스킬 폴더 안의 `examples/{domain}/`(`.claude/skills/humanize-book-korean/examples/`)에 분리합니다. 본 저장소에는 한 가지 도메인 예시가 들어 있습니다:
...
│   │   │   ├── book-extra-rules.md # P~Z (한국 IT 책 보강)
│   │   │   ├── user-style-traits.md
│   │   │   ├── ai-tell-taxonomy.md
│   │   │   ├── rewriting-playbook.md
│   │   │   ├── scholarship.md
│   │   │   ├── metrics.py · metrics_v2.py
│   │   │   └── baseline.json · baseline_v2.json
│   │   └── examples/               # 도메인별 자산 (스킬 폴더 안 — 플러그인/이식 호환)
│   │       ├── README.md
...
│
├── scripts/                         # 정규식 도구 + 설치
│   ├── apply-tone-fast.py          # 일괄 변환 (수 초)
│   ├── check-tone.py               # 위반 자동 점검
│   ├── install.sh                  # 설치 (macOS/Linux)
│   └── install.ps1                 # 설치 (Windows)
│
└── tests/                           # 회귀 테스트 (fixtures 미구현)
    └── fixtures/
```
