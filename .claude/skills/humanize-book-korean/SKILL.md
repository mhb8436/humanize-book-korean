...
# Humanize Book Korean — 한국 IT 책 톤 윤문 (v0.1)

> **본 스킬은 im-not-ai (https://github.com/epoko77-ai/im-not-ai, MIT) v2.0을 base로 한국 IT 책 작업에 특화한 fork입니다.** A~J 카테고리는 원본 유지, P~W 카테고리(한국 IT 책 특유의 영어식 패턴) 보강.

> **v0.1 (2026-05-27) — 초기 릴리스 + Monolith Fast Path**
...
```
humanize-book-korean v0.1 — {fast|strict} 모드 / run_id: {YYYY-MM-DD-NNN}
```

### 한국 IT 책 톤 적용 (★ 본 스킬의 특화)

본 스킬은 원본 humanize-korean의 룰북에 더해 한국 IT 책 특유의 패턴을 보강합니다.

**필수 참조 자료** (Phase 1에서 먼저 로드):
1. `references/quick-rules.md` — A~J 카테고리 (im-not-ai 원본)
2. `references/book-extra-rules.md` — P~W 카테고리 (한국 IT 책 특화)
3. `references/user-style-traits.md` — 한국 IT 책 톤 8가지 특징
4. `examples/{domain}/domain-glossary.md` — 도메인 용어 표준 (사용자 책마다 다름)
5. `examples/{domain}/persona.md` — 페르소나 보존 규칙 (사용자 책마다 다름)

`{domain}`은 cwd의 `_workspace/{run_id}/book-context.yaml`에서 결정하거나, 없으면 사용자 인자로 받습니다.

본 스킬의 Do-NOT 추가:
- **도메인 용어 보존**: domain-glossary.md에 명시된 표준 용어는 절대 다른 표현으로 변경하지 않음
- **페르소나 보존**: persona.md에 명시된 인물 정보(이름·소속·경력)는 100% 보존
- **사용자 자바책 톤 매치**: examples/{domain}/reference-style/ 의 비슷한 단락과 톤이 다르면 그 톤에 맞춤
